...
const delete_user_by_username = async (req, res) => {
  try {
    let user_id = await get_user_id(req); // Get logged-in user ID
    console.log("Logged-in User ID:", user_id);

    // Fetch the logged-in user's details
    let loggedInUser = await UserModel.findOne({ where: { id: user_id } });

    // Ensure the logged-in user exists
    if (!loggedInUser) {
      return res
        .status(403)
        .json({ message: "Logged-in user not found", ok: false });
    }

    // Check if the logged-in user is an admin
    const isAdmin = loggedInUser.account_type === "admin";
    console.log("Is logged-in user admin?", isAdmin);

    let { username } = req.body;

    // Fetch the user to be deleted by username
    let userToDelete = await UserModel.findOne({
    ...
    console.log("User to delete account type:", userToDelete.account_type);

    // If logged-in user is not an admin and the user to delete is an admin, deny the operation
    if (!isAdmin && userToDelete.account_type === "admin") {
      return res
        .status(403)
        .json({ message: "Cannot delete an admin user", ok: false });
    }

    // Proceed with deletion if the above checks pass
    ...
    if (!deletedUser) {
      return res
        .status(400)
        .json({ message: "Failed to delete user", ok: false });
    }
